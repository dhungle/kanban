...
public class StreamIT {
    ...
    public void helloWorld() throws Exception {
        String answer = await(boardService.hello("foo").invoke());
        assertEquals("Hello, foo!", answer);
        await(boardService.useGreeting("bar").invoke(new GreetingMessage("Hi")));
        String answer2 = await(boardService.hello("bar").invoke());
